...
def strip_scheme(url):
    ...
    return parsed.replace("/", "_").rstrip("/")


class Parser:
    # 
    def __init__(self, templates_path="templates", caching: int=1, caching_duration: int=12*60*60, cache_folder: str="cache", mode: str='web-service'):
    ...
    def get_and_check_url_response(url):
        ...
        if not st_code == requests.codes.ok:
            raise Exception(st_code, "Request return {} status code".format(str(st_code)))
    ...
    def file_exists_in_cache(self, cached_file_path: str) -> bool:
        return os.path.isfile(cached_file_path) and (time.time() - os.path.getmtime(cached_file_path) < self.caching_duration)
...
def main():
    ...
    arg_parser.add_argument('--width', type=int, default=200, help='Width of the output.')
    arg_parser.add_argument('--img', type=int, default=1, help='Include images in the output.')
